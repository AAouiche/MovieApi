...
namespace Application.Handlers.Account
{
    public class Login
    {
        ...
        public class Handler:IRequestHandler<LoginCommand , Result<UserDTO>>
        {
            ...
            public Handler(IAccountRepository accountRepository,UserManager<ApplicationUser> userManager,TokenService service)
            {
            ...
            public async Task<Result<UserDTO>> Handle(LoginCommand command,CancellationToken cancellationToken)
            {
                ...
                if (user == null) 
                {
                    return Result<UserDTO>.Failure("The user does not exist", ErrorCode.Unathorized);
                        
                };
                var checkedPass = await _userManager.CheckPasswordAsync(user,command.LoginDetails.Password);
                if (!checkedPass)
                {
                    return Result<UserDTO>.Failure("Incorrect Password", ErrorCode.Unathorized);
                }
